...
start:;
	...
	t = minuty();
	if(sekundy()>=60)printf("%d minut %d sekund", minuty(), sekundy()-(60*t));// implementacja czasu na dole planszy
	else printf("%d minut %d sekund", minuty(), sekundy());
	goto start;
	lost:;
	if(x==0 || x==6)printf("koniec uwazaj na bande twoj czas to %d minut i %d sekund!!\n",minuty(), sekundy());
	else printf("przegrales, nastepnym razem bedzie lepiej :) twoj czas to  %d minut i %d sekund\n", minuty(), sekundy());
